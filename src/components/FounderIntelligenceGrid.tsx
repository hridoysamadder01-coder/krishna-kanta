...
export function FounderIntelligenceGrid({ locale }: { locale: Locale }) {
  return (
    <section className="bg-ivory py-24 text-ink sm:py-32" aria-labelledby="intelligence-heading">
      <div className="mx-auto max-w-wide px-5 sm:px-8">
        <Reveal>
          <SectionLabel tone="light">{locale === "bn" ? "চারটি স্তম্ভ" : "Four Pillars"}</SectionLabel>
          <EditorialHeading id="intelligence-heading" className="mt-6 max-w-3xl text-ink">
            {t(home.intelligenceTitle, locale)}
          </EditorialHeading>
        </Reveal>

        <ul className="mt-16 grid gap-px border border-ink/10 bg-ink/10 sm:grid-cols-2" role="list">
          {intelligencePanels.map((panel, i) => (
            <Reveal
              as="li"
              key={panel.id}
              delay={i * 0.08}
              className="group bg-paper p-8 transition-colors duration-500 hover:bg-ivory-deep sm:p-12"
            >
              <p className="font-display text-sm text-gold-muted" aria-hidden="true">
                {editorialIndex(i + 1, locale)}
              </p>
              <h3 className="font-display mt-6 text-2xl font-medium text-ink sm:text-[1.7rem]">
              ...
              />
              <p className="mt-5 max-w-md text-[0.98rem] leading-[1.8] text-ink/70">
                {t(panel.text, locale)}
              </p>
            </Reveal>
          ))}
