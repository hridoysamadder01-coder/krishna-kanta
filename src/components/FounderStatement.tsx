...
export function FounderStatement({ locale }: { locale: Locale }) {
  ...
  return (
    <section className="bg-paper py-24 text-ink sm:py-32" aria-labelledby="intro-heading">
      <div className="mx-auto max-w-wide px-5 sm:px-8">
        <div className="grid gap-14 lg:grid-cols-12">
          <Reveal className="lg:col-span-7">
            <SectionLabel tone="light">{t(ui.editorialLabel, locale)}</SectionLabel>
            <EditorialHeading id="intro-heading" className="mt-6 text-ink">
              {t(home.introHeadline, locale)}
            </EditorialHeading>
            <div className="mt-8 max-w-measure space-y-6 text-[1.02rem] leading-[1.85] text-ink/75">
          ...
          {pullQuote && (
            <Reveal delay={0.15} className="flex items-center lg:col-span-5">
              <div className="border-l border-gold-muted/50 pl-8">
                <p className="font-display text-[clamp(1.5rem,3.2vw,2.3rem)] font-medium italic leading-[1.35] text-ink/85">
                  {t(pullQuote.text, locale)}
                </p>
                <p className="tracking-label mt-5 text-[0.6rem] uppercase text-ink/45">
